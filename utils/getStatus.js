  ...
  getStatus: async (server, color) => {
    ...
    if (err === 1 || !data) {
      const errEmbed = new MessageEmbed()
        .setColor('ff0000')
        .setTitle('Error')
        .setDescription(`${server.ip}:${server.port} n'a pas répondu après 5 tentatives.`)
        .setTimestamp();
      return errEmbed;
    }
    ...
    let output, output2;
    if (players.length === 0) output = 'Rien';
    ...
    let serverEmbed = new MessageEmbed()
      ...
      .addFields(
        { name: 'Serveur IP', value: `${server.ip}:${server.port}`, inline: true },
        ...
        { name: 'Joueurs', value: `${data.players.length}/${data.maxplayers}`, inline: true }
      )
      .setTimestamp();
    if (output2) serverEmbed.addField('Joueurs en ligne', '```' + output2 + '```');
